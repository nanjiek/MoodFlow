...
def train(dataset: Path, output_dir: Path, test_dataset: Path | None = None) -> dict[str, object]:
    ...
    if test_dataset is not None:
        ...
        label_counts = {k: int(v) for k, v in pd.concat([df["label"], test_df["label"]]).value_counts().to_dict().items()}
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="model_service/artifacts/baseline-clean-v4")
